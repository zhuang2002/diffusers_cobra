...
from ..attention import BasicTransformerBlock, CausalTransformerBlock
...
class CausalSparseDiTControlModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    r"""
    ...
    _no_split_modules = ["CausalTransformerBlock", "CausalPatchEmbed"]
# {
...
# }
    ...
    def __init__(
        ...
        self.pos_embed = CausalPatchEmbed(
            ...
            in_channels=self.config.in_channels + cond_chanels,
        ...
        adapter_list = []
        for _ in range(self.config.num_layers//2):
            self.after_proj = Linear(self.inner_dim, self.inner_dim) 
            init.zeros_(self.after_proj.weight)
            init.zeros_(self.after_proj.bias)
            adapter_list.append(self.after_proj)

        self.adapter_linear = nn.ModuleList(adapter_list)
    ...
    def forward(
        ...
        # ref_hidden_states: torch.Tensor, # (b, n_ref, c, h, w)
        # n_ref_lists: List[List[int]],
        ...
        output_list = []
        for block, zero_adapter in zip(self.transformer_blocks, self.adapter_linear):
            ...
            output_list.append([zero_adapter(hidden_states)])
