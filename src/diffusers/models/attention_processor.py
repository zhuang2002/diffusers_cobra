...
class CausalAttention(nn.Module):
    ...
    Parameters:
        ...
        _from_deprecated_attn_block (`bool`, *optional*, defaults to `False`):
            Set to `True` if the attention block is loaded from a deprecated state dict.
        processor (`AttnProcessor`, *optional*, defaults to `None`):
            The attention processor to use. If `None`, defaults to `CausalAttnProcessor2_0` if `torch 2.x` is used and
            `AttnProcessor` otherwise.
    """
    ...
    def __init__(
        ...
        if processor is None:
            processor = (
                CausalAttnProcessor2_0() if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor()
            )
        self.set_processor(processor)
    ...
    def forward(
        ...
        # cond_hidden_states: Optional[torch.Tensor] = None, # (b, l, c)
        ref_hidden_states: Optional[torch.Tensor] = None, # (b, n_ref, l, c)
        encoder_hidden_states: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        **cross_attention_kwargs,
    ) -> torch.Tensor:
        ...
        return self.processor(
            self,
            hidden_states, # (b, l, c)
            # cond_hidden_states, # (b, l, c)
            ref_hidden_states, # (b, n_ref, l, c)
            encoder_hidden_states=encoder_hidden_states,
            attention_mask=attention_mask,
            **cross_attention_kwargs,
        )
...
class CausalAttnProcessor2_0:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("CausalAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
    ...
    def __call__(
        ...
        encoder_hidden_states: Optional[torch.Tensor] = None, # 始终为None
        attention_mask: Optional[torch.Tensor] = None,
        temb: Optional[torch.Tensor] = None,
        *args,
        **kwargs,
        ...
        flat_ref_hidden_states = rearrange(ref_hidden_states, 'b n_ref l c -> (b n_ref) l c')
        residual = hidden_states
        # residual_cond = cond_hidden_states
        residual_ref = flat_ref_hidden_states

        if attn.spatial_norm is not None:
            hidden_states = attn.spatial_norm(hidden_states, temb)
            # cond_hidden_states = attn.spatial_norm(cond_hidden_states, temb)
            flat_ref_hidden_states = attn.spatial_norm(flat_ref_hidden_states, temb)
        ...
        # if attn.group_norm is not None:
        #     hidden_states = attn.group_norm(hidden_states.transpose(1, 2)).transpose(1, 2)
        #     cond_hidden_states = attn.group_norm(cond_hidden_states.transpose(1, 2)).transpose(1, 2)
        #     flat_ref_hidden_states = attn.group_norm(flat_ref_hidden_states.transpose(1, 2)).transpose(1, 2)
        #     print('group_norm')

        # query = attn.to_q(hidden_states)
        # query_ref = attn.to_q(flat_ref_hidden_states)

        # reshape_flat_ref_hidden_states = rearrange(flat_ref_hidden_states, '(b n_ref) l c -> b (n_ref l) c', n_ref=N_ref)

        # encoder_hidden_states = torch.cat([hidden_states, reshape_flat_ref_hidden_states], dim=1)
        encoder_hidden_states = hidden_states
        # encoder_hidden_states_cond = cond_hidden_states
        encoder_hidden_states_ref = flat_ref_hidden_states # (b, n_ref), l, c



        query = attn.to_q(hidden_states) # (b, l, c)
        key_0 = attn.to_k(encoder_hidden_states) # (b, l, c)
        value_0 = attn.to_v(encoder_hidden_states) # (b, l, c)

        # query_cond = attn.to_q(cond_hidden_states) # (b, l, c)
        # key_cond = attn.to_k(encoder_hidden_states_cond) # (b, l, c)
        # value_cond = attn.to_v(encoder_hidden_states_cond) # (b, l, c)

        query_ref = attn.to_q(flat_ref_hidden_states) # (b, n_ref), l, c
        # print('encoder_hidden_states_ref', encoder_hidden_states_ref.shape)
        key_ref = attn.to_k(encoder_hidden_states_ref) # (b, n_ref), l, c
        value_ref = attn.to_v(encoder_hidden_states_ref) # (b, n_ref), l, c

        reshape_key_ref = rearrange(key_ref, '(b n_ref) l c -> b (n_ref l) c', n_ref=N_ref)
        reshape_value_ref = rearrange(value_ref, '(b n_ref) l c -> b (n_ref l) c', n_ref=N_ref)

        key = torch.cat([key_0, reshape_key_ref], dim=1) # (b, n_ref*l + l1, c)
        value = torch.cat([value_0, reshape_value_ref], dim=1) # (b, n_ref*l + l1, c)

        # print('key',key.shape, key_0.shape, key_cond.shape, reshape_key_ref.shape)
        # print('value',value.shape, value_0.shape, value_cond.shape, reshape_value_ref.shape)

        # key_cond = torch.cat([key_cond, key_0], dim=1)
        # value_cond = torch.cat([value_cond, value_0], dim=1)

        # key_ref = key.unsqueeze(1).repeat(1, N_ref, 1, 1)
        # key_ref = rearrange(key_ref, 'b n_ref l c -> (b n_ref)  l c', n_ref=N_ref)
        # value_ref = value.unsqueeze(1).repeat(1, N_ref, 1, 1)
        # value_ref = rearrange(value_ref, 'b n_ref l c -> (b n_ref) l c', n_ref=N_ref)

        # key_for_ref = key_0.unsqueeze(1).repeat(1, N_ref, 1, 1).squeeze(0)
        # value_for_ref = value_0.unsqueeze(1).repeat(1, N_ref, 1, 1).squeeze(0)
        # print('key_for_ref',key_for_ref.shape)
        # print('value_for_ref',value_for_ref.shape)
        # print('key_ref',key_ref.shape)
        # print('value_ref',value_ref.shape)

        # key_ref = torch.cat([key_ref, key_for_ref], dim=1)
        # value_ref = torch.cat([value_ref, value_for_ref], dim=1)


        # print('key',key.shape, key_ref.shape, key_cond.shape)
        # print('value',value.shape, value_ref.shape, value_cond.shape)
        ...
        # query_cond = query_cond.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
        # key_cond = key_cond.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
        # value_cond = value_cond.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)

        query_ref = query_ref.view(batch_size*N_ref, -1, attn.heads, head_dim).transpose(1, 2)
        key_ref = key_ref.view(batch_size*N_ref, -1, attn.heads, head_dim).transpose(1, 2)
        value_ref = value_ref.view(batch_size*N_ref, -1, attn.heads, head_dim).transpose(1, 2)

        if attn.norm_q is not None:
            query = attn.norm_q(query)
            # query_cond = attn.norm_q(query_cond)
            query_ref = attn.norm_q(query_ref)
        if attn.norm_k is not None:
            key = attn.norm_k(key)
        ...
        # hidden_states_cond = F.scaled_dot_product_attention(
        #     query_cond, key_cond, value_cond, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        # ) 

        hidden_states_ref = F.scaled_dot_product_attention(
            query_ref, key_ref, value_ref, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        ) 

        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        hidden_states = hidden_states.to(query.dtype)

        # hidden_states_cond = hidden_states_cond.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        # hidden_states_cond = hidden_states_cond.to(query.dtype)

        hidden_states_ref = hidden_states_ref.transpose(1, 2).reshape(batch_size*N_ref, -1, attn.heads * head_dim)
        ...
        # hidden_states_cond = attn.to_out[0](hidden_states_cond)
        # hidden_states_cond = attn.to_out[1](hidden_states_cond)

        hidden_states_ref = attn.to_out[0](hidden_states_ref)
        hidden_states_ref = attn.to_out[1](hidden_states_ref)


        if attn.residual_connection:
            hidden_states = hidden_states + residual
            # hidden_states_cond = hidden_states_cond + residual_cond
            hidden_states_ref = hidden_states_ref + residual_ref

        hidden_states = hidden_states / attn.rescale_output_factor
        # hidden_states_cond = hidden_states_cond / attn.rescale_output_factor
        hidden_states_ref = hidden_states_ref / attn.rescale_output_factor

        hidden_states_ref = rearrange(hidden_states_ref, '(b n_ref) l c -> b n_ref l c', n_ref=N_ref)
        return hidden_states, hidden_states_ref


@maybe_allow_in_graph
class CausalKVcacheAttention(nn.Module):
    r"""
    A cross attention layer.
    ...
    Parameters:
        ...
        _from_deprecated_attn_block (`bool`, *optional*, defaults to `False`):
            Set to `True` if the attention block is loaded from a deprecated state dict.
        processor (`AttnProcessor`, *optional*, defaults to `None`):
            The attention processor to use. If `None`, defaults to `CausalKVcacheAttnProcessor2_0` if `torch 2.x` is used and
            `AttnProcessor` otherwise.
    """
    ...
    def __init__(
        ...
        if processor is None:
            processor = (
                CausalKVcacheAttnProcessor2_0() if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor()
            )
        self.set_processor(processor)
...
class CausalKVcacheAttnProcessor2_0:
    r"""
    Processor for implementing scaled dot-product attention (enabled by default if you're using PyTorch 2.0).
    """
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("CausalKVcacheAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")

    def __call__(
        self,
        attn: CausalKVcacheAttention,
        hidden_states: torch.Tensor,
        ref_hidden_states: Optional[torch.Tensor] = None,
        encoder_hidden_states: Optional[torch.Tensor] = None, # 始终为None
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if no_cache:
            key = torch.cat([key_0, reshape_key_ref], dim=1) # (b, n_ref*l + l1, c)
            value = torch.cat([value_0, reshape_value_ref], dim=1) # (b, n_ref*l + l1, c)
            output_K_cache = reshape_key_ref
            output_V_cache = reshape_value_ref
        else:
            key = torch.cat([key_0, K_cache], dim=1) # (b, n_ref*l + l1, c)
            value = torch.cat([value_0, V_cache], dim=1) # (b, n_ref*l + l1, c)
        ...
        # ) 
        if no_cache:
            # print('query_ref',query_ref.dtype, query_ref.shape)
            # print('key_ref',key_ref.dtype, key_ref.shape)
            # print('value_ref',value_ref.dtype, value_ref.shape)
            hidden_states_ref = F.scaled_dot_product_attention(
                query_ref, key_ref, value_ref, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
            ) 
        ...
        if no_cache:
            hidden_states_ref = rearrange(hidden_states_ref, '(b n_ref) l c -> b n_ref l c', n_ref=N_ref)
            return hidden_states, hidden_states_ref, output_K_cache, output_V_cache
        else:
